...
public class A3  extends JFrame {
		...
		public void actionPerformed(ActionEvent e){
			...
			if (s != null) {
				if(s instanceof NestedShape){
					panel.addShapeNode((NestedShape)s);
				}
				else {JOptionPane.showMessageDialog(A3.this,"ERROR: Must select a NestedShape node.");
				}
		
			}else JOptionPane.showMessageDialog(A3.this,"ERROR: No node selected." );
		}
	}
	class RemoveListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if (s != null) {
				if(s != panel.getRoot()){
					panel.removeNodeFromParent((NestedShape)s);
				}
				else {JOptionPane.showMessageDialog(A3.this,"ERROR: Must not remove the root.");
				}
	...
	class TreeNodeSelectionListener implements TreeSelectionListener{
		public void valueChanged(TreeSelectionEvent e){
			Object s = tree.getLastSelectedPathComponent();
			if (s != null) {
			    TreePath path = new TreePath(((Shape) s).getPathToRoot((Shape)s,0));
				tree.expandPath(path);
				panel.reload((Shape)s);
